...
- Visual baselines are **OS/font-rendering sensitive** and must only ever be regenerated via the `update-visual-snapshots` job (`workflow_dispatch` on [`design-system-ci.yml`](.github/workflows/design-system-ci.yml:9): `gh workflow run design-system-ci.yml --ref <branch> -f update-snapshots=true`) — **never locally, and never via `act`.** No local Docker image, however well-matched, reliably bit-matches GitHub's hosted runner's font-rendering stack (fontconfig/freetype/harfbuzz drift independently of the pinned Playwright/Chromium version); `act`'s runner images are a close approximation, not a guarantee. This isn't a preference, it's the difference between baselines that pass `test-visual` in the real `ci` job and ones that fail it on a ~0.03 diff that looks like a regression but is really just environment mismatch — this has already happened once (PR #6). The job pushes a commit with the regenerated PNGs directly to the branch it was dispatched against (never `main` — it refuses to run there); **pull before continuing local work on that branch**, or your next push will be rejected as non-fast-forward. Review the commit's image diff in the PR's Files Changed tab before merging — the job commits mechanically, it does not review, and a diff worth committing is not automatically a diff worth merging.
- `maxDiffPixelRatio: 0.02` absorbs same-runner font-hinting drift (∼1%) across CI runs — it is not calibrated for cross-environment drift (e.g. a locally- or `act`-generated baseline compared against the real runner) and should not be raised to accommodate that; fix the baseline's origin instead of widening the tolerance for every other story in the suite.
