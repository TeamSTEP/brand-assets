...
- Reviewing a PR against `/design-system` (this repo's CLAUDE.md), not generic best practice
...
1. **Token coverage** — grep component CSS for hex/rgb literals outside `src/tokens/`; cross-check against what `pnpm run lint` actually catches (color only, today — spacing/radius are unenforced gaps, not passes).
...
Before writing code, answer these — they map directly to CLAUDE.md's standing rules:
...
- **Does it break at any of the three viewports by construction, or does it need a written "remember to test this" note?** If the latter, the responsiveness isn't actually in the component yet — CLAUDE.md requires it live in the component's own CSS (`clamp()`/container queries), not a consumer's responsibility.

## Common Mistakes

| Mistake | Reality |
|---|---|
| "Lint passed, so the tokens are fine" | Lint only checks color properties today. Spacing/radius literals pass silently — check by eye until those tiers are lint-enforced. |
